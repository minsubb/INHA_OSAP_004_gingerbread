...
public:
    //생성자와 소멸자
    Node(int key) : key_(key), height_(1), sub_size_(0) { left_child_ = right_child_ = NULL; }
    ~Node() {}
...
int AVLTree::GetRank(Node* node, int x) {
    ...
    else if (x > node->get_key()) { //기준 키값이 현재 키값보다 크면 왼쪽 서브 트리 사이즈를 더해준 후 오른쪽 자식만 탐색한다
        (node->get_left_child() != NULL) ?
            rank += node->get_left_child()->get_sub_size() + 2:
            rank ++;
...
Node* AVLTree::Insert(Node* node, int key) {
    ...
    // 새로운 자식노드가 추가되었으므로 서브트리 재계산
    node->set_sub_size(node->get_sub_size()+1);
...
Node* AVLTree::DeleteNode(Node* root, int key) {
    ...
    if (key < root->get_key()) {
        ...
        root->set_sub_size(root->get_sub_size()-1);
    }
...
Node* AVLTree::RightRotate(Node* zNode) {
    ...
    //회전하면서 서브트리 사이즈가 달라졌으므로 sub_size 재계산
    int zNodeSize = zNode->get_sub_size();
    (yNode->get_left_child() != NULL) ?
        zNode->set_sub_size(zNode->get_sub_size()-yNode->get_left_child()->get_sub_size()-2):
        zNode->set_sub_size(zNode->get_sub_size()-1);
    yNode->set_sub_size(zNodeSize);
    ...
    //cout<<"\n"<<yNode->get_key()<<" subsize : "<<yNode->get_sub_size()<<endl;
    //cout<<"\n"<<zNode->get_key()<<" subsize : "<<zNode->get_sub_size()<<endl;
...
Node* AVLTree::LeftRotate(Node* zNode) {
    ...
    //회전하면서 서브트리 사이즈가 달라졌으므로 sub_size 재계산
    int zNodeSize = zNode->get_sub_size();
    (yNode->get_right_child() != NULL) ?
        zNode->set_sub_size(zNode->get_sub_size()-yNode->get_right_child()->get_sub_size()-2):
        zNode->set_sub_size(zNode->get_sub_size()-1);
    yNode->set_sub_size(zNodeSize);
